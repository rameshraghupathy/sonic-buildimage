...
$(info "SECURE_UPGRADE_PROD_DETACHED_SIGNING_TOOL": "$(SECURE_UPGRADE_PROD_DETACHED_SIGNING_TOOL)")
$(info "SECURE_UPGRADE_PROD_DETACHED_TOOL_ARGS": "$(SECURE_UPGRADE_PROD_DETACHED_TOOL_ARGS)")
$(info "NO_SHIM"                         : "$(NO_SHIM)")
$(info "ONIE_IMAGE_AB_PARTITION"         : "$(ONIE_IMAGE_AB_PARTITION)")
$(info "SONIC_IMMUTABLE_FS"              : "$(SONIC_IMMUTABLE_FS)")
$(info "PLATFORM_HW_SKU"                 : "$(PLATFORM_HW_SKU)")
...
$(addprefix $(TARGET_PATH)/, $(SONIC_INSTALLERS)) : $(TARGET_PATH)/% : \
	...
	export sonic_su_prod_detached_signing_tool="/sonic/scripts/$(shell basename -- $(SECURE_UPGRADE_PROD_DETACHED_SIGNING_TOOL))"
	export onie_image_ab_partition="$(ONIE_IMAGE_AB_PARTITION)"
	export sonic_immutable_fs="$(SONIC_IMMUTABLE_FS)"
	export no_shim="$(NO_SHIM)"
	export platform_hw_sku="$(PLATFORM_HW_SKU)"
	...
	$(foreach dep_machine, $($*_MACHINE) $($*_DEPENDENT_MACHINE), \
		...
		PLATFORM_HW_SKU=$(PLATFORM_HW_SKU) \
		NO_SHIM=$(NO_SHIM) \
		SONIC_ENFORCE_VERSIONS=$(SONIC_ENFORCE_VERSIONS) \
		TRUSTED_GPG_URLS=$(TRUSTED_GPG_URLS) \
		BUILD_PACKAGES_URL=$(BUILD_PACKAGES_URL) \
		...
		SECURE_UPGRADE_PROD_DETACHED_SIGNING_TOOL="$(SECURE_UPGRADE_PROD_DETACHED_SIGNING_TOOL)" \
		SECURE_UPGRADE_PROD_DETACHED_TOOL_ARGS="$(SECURE_UPGRADE_PROD_DETACHED_TOOL_ARGS)" \
